...
// Landscape only: portrait on a touch device shows the rotate gate (see 'gate' shot below).
const sizes = { landscape: [844, 390] };
...
for (const [name, [w, h]] of Object.entries(sizes)) {
  for (const lang of ['ko', 'en']) {
    ...
    await page.evaluate(() => document.querySelector('.single-toggle').click()); await sleep(900); await shot("5-chars");
...
// Portrait gate check.
{
  ...
  await page.evaluateOnNewDocument(() => { try { localStorage.setItem('tkr.lang', 'ko'); localStorage.setItem('tkr.nudged', '1'); } catch {} });
  await page.goto(`${BASE}/?auto=1`, { waitUntil: 'load' });
  await page.waitForFunction(() => window.__turboKartRush);
  await sleep(600);
  await page.screenshot({ path: `${OUT}/portrait-ko-gate.png` });
  await page.close();
}
await browser.close();
