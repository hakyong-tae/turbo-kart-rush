...
- **(online-multi)** `RaceSettings.online?: OnlineRaceConfig`, `NetKartPose`, `IKart.applyNetState?` — host-authoritative online races. Workstream **G — Net**: `src/net/**` (transport, protocol, roster, host/client sessions, lobby, online controller) + room functions in `server.js`; UI `src/ui/OnlinePanel.ts`. `Game` uses `r.localKartId` instead of `karts[0]`.
- **(item-sync)** `IItemManager.setNetMode?/getNetItems?/applyNetItems?`, `NetHazard`, `NetItems`, `NetKartPose` status+item fields, `OnlineRaceConfig.items`, `Kart.applyNetStatus` — host-authoritative item sync (`docs/superpowers/specs/2026-09-06-item-sync-design.md`).
- **(host-migration)** Snapshot header `hostEpoch`; `RaceManager.adoptFromKarts(raceTime, phase)`; `HostSession/ClientSession.handleMessage` (OnlineController owns the transport handler); `pickNextHost`. Game title is now DRIFT DASH GP (`GAME_TITLE`).
- **(settings-mixer)** `IAudioEngine.setMusicVolume/setSfxVolume` + `musicVolumeLevel/sfxVolumeLevel` — independent music / SFX levels (SFX covers the sfx, engines and ui buses). Settings panel sliders replace the mute checkbox; the `M` hotkey mute is unchanged. Stored in `localStorage tkr.vol.{music,sfx}`.
- **(kart-bodies)** `src/kart/bodies.ts` — one `KartBodySpec` per character (wheel sizes, lift, exhaust layout, plate positions, `build(kit)` chassis/spoiler/trim). `src/kart/modelUtils.ts` holds the shared geometry helpers (`Batch`, `profileChassis`, …). `buildKartModel()` signature and `KartModelPartsEx` handles unchanged; physics/collision untouched (`docs/superpowers/specs/2026-09-07-kart-bodies-design.md`).
- **(gameplay-2)** `ItemType` gains `'magnet'` (appended → net item codes unchanged), `BoostSource` gains `'slipstream' | 'magnet'`, `KartState` gains `draftCharge/isDrafting/magnetTargetId/magnetTimer`, `IKart.applyMagnet()`, events `kart:draftStart/magnetStart/magnetEnd`, `BALANCE.slipstream` + `BALANCE.items.magnet*`. Star karts are immovable in kart-vs-kart collisions.
- **(gameplay-3)** `KartState.startCharge`, `IKart.getExhaustAnchors?()` + `ExhaustAnchor`, events `kart:draftEnd` / `kart:startOvercharge`, `BALANCE.race.startChargeGood/startChargePerfect` (replace startBoostWindow/WeakWindow). RaceManager owns the launch boost (Kart.setFrozen no longer boosts). ParticleSystem receives `IKart` per emitter (flames on the real pipes, grid-charge smoke, slipstream streaks). HUD `update()` takes `hazards` for the rear-view mirror.
- **(gameplay-4)** `RaceMode` (`solo | teamPoints | teamFirst`), `Team`, `RaceSettings.mode?`, `RaceStanding.team?`, `race:finish.retired?`, `BALANCE.race.retireSeconds` (replaces finishGraceSeconds). `src/core/teams.ts` = team rules (even kart ids RED / odd BLUE, 10/8/6/5/4/3/2/1 points, P1 rule). RaceManager retires everyone still racing 10 s after the winner (finishTime -1). HUD `setMode()`, ResultsScreen `show(standings, mode)`, Minimap `setTeamColors()`, team chevron sprites in Game.
- **(gameplay-5)** Item ids renamed `mushroom→nitro`, `triple_mushroom→triple_nitro`, `golden_mushroom→overdrive` (same `ALL_ITEM_TYPES` slots → net codes unchanged); `BoostSource` `nitro | overdrive`; `KartState.overdriveTimer`; `BALANCE.items.overdriveDuration`. Lobby title (`.lobby`), landscape gate (`.rotate-gate`), HUD standings board + top-right stacked item slot. Engine voice retuned toward an F1 scream (engine.ts constants).
- **(structure)** `src/styles/*.css` (ordered split of the old style.css), `src/kart/assists/{slipstream,magnet}.ts`, `src/ui/hud/{StandingsBoard,MirrorPanel,TeamTally,ChargeGauge,DraftMeter}.ts`, `src/game/{TeamMarkers,OrientationGate,settingsStore}.ts`. `BALANCE.slipstream.bonus` (leader→follower weight-class matrix) replaces `speedBonus`. Module map: `docs/ARCHITECTURE.md`.
